...
import { Leaf, Tractor, Wheat, Sun, CloudRain, ShoppingCart, TrendingUp, Package, Truck, Wallet } from 'lucide-react';
...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // If the user has a role in metadata, check if it matches the current login attempt
            // Role verification removed to allow flexible login
            /*
            if (userRole && userRole !== role) {
                await supabase.auth.signOut();
                throw new Error(`This account is registered as a ${userRole}. Please use the ${userRole} login.`);
            }
            */

            // 3️⃣ Update Context & Redirect
            login(data.user);
            navigate('/');
        ...
        <div className={`min-h-screen flex items-center justify-center p-4 ${theme.pageBg} relative overflow-hidden`}>
            ...
            <div className={`max-w-md w-full p-8 border ${theme.borderColor} rounded-2xl shadow-2xl ${theme.cardBg} relative z-10 animate-fade-in-up`}>
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-600">Welcome to Bhumi Putra</p>
                </div>
                ...
                <p className="mt-6 text-center text-gray-600">
                    ...
                    <Link to={`/register?role=${role}`} className={`${theme.linkColor} ${theme.linkHover} hover:underline font-semibold`}>
                        Register here
                    </Link>
                </p>
            </div>
